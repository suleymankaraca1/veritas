"""
ResearcherD -- SimpleAdapter, no LangGraph.
...
from utils.tavily_client import search as tavily_search, search_academic as tavily_search_academic
...
AGENT_ID = os.getenv("RESEARCHER_D_AGENT_ID")
API_KEY = os.getenv("RESEARCHER_D_API_KEY")
...
ANALYSIS_PROMPT = """You are an ACADEMIC fact-checker specializing in peer-reviewed sources.
You have academic/scientific search results for the claim below.
Evaluate the claim's accuracy based strictly on academic and scientific evidence.
...
Analysis: [2-3 sentences: what does the academic/scientific evidence say? cite specific studies or findings if available]
Support Score: [integer 0-100 -- 100=definitively proven true by academic sources, 0=definitively proven false, 50=no academic consensus]
Reliable Sources: [source URLs if any, comma-separated, or "none found"]"""
...
def _research_one(claim: str) -> str:
    logger.info(f"[ResearcherD] Researching: {claim[:70]}...")
    try:
        search_results = tavily_search_academic(claim, max_results=5)
...
def _research_and_send(task_id: str, claims: list[str]):
    ...
    logger.info(f"[ResearcherD] TASK_ID={task_id} -- findings sent to SourceValidator")


class ResearcherDAdapter(SimpleAdapter):
    async def on_message(
        ...
        *,
        ...
        if "researcherd" not in content_lower:
            return
        ...
        if not task_id:
            logger.warning("[ResearcherD] TASK_ID not found")
            return

        logger.info(f"[ResearcherD] TASK_ID={task_id} -- {len(claims)} claims received")
...
def start():
    def _run():
        adapter = ResearcherDAdapter()
        agent = Agent.create(adapter=adapter, agent_id=AGENT_ID, api_key=API_KEY)
        logger.info("[ResearcherD] Started (SimpleAdapter / Tavily+LLM)")
        asyncio.run(agent.run())

    t = threading.Thread(target=_run, daemon=True, name="researcher-d")
    t.start()
