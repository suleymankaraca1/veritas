"""
ResearcherC -- SimpleAdapter, no LangGraph.
...
AGENT_ID = os.getenv("RESEARCHER_C_AGENT_ID")
API_KEY = os.getenv("RESEARCHER_C_API_KEY")
...
def _research_one(claim: str) -> str:
    logger.info(f"[ResearcherC] Researching: {claim[:70]}...")
    try:
...
def _research_and_send(task_id: str, claims: list[str]):
    ...
    logger.info(f"[ResearcherC] TASK_ID={task_id} -- findings sent to SourceValidator")


class ResearcherCAdapter(SimpleAdapter):
    async def on_message(
        ...
        *,
        ...
        if "researcherc" not in content_lower:
            return
        ...
        if not task_id:
            logger.warning("[ResearcherC] TASK_ID not found")
            return

        logger.info(f"[ResearcherC] TASK_ID={task_id} -- {len(claims)} claims received")
...
def start():
    def _run():
        adapter = ResearcherCAdapter()
        agent = Agent.create(adapter=adapter, agent_id=AGENT_ID, api_key=API_KEY)
        logger.info("[ResearcherC] Started (SimpleAdapter / Tavily+LLM)")
        asyncio.run(agent.run())

    t = threading.Thread(target=_run, daemon=True, name="researcher-c")
    t.start()
